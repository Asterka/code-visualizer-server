...
exports.decompileUserProject = async (proj_name, token, project_token) => {
  await exec(
    `java -jar ./project-files/jd-cmd/jd-cli.jar ./res/upload-dir/${token}/${project_token}/${proj_name} -ods ./reports/${token}/${project_token} && cd ${__dirname}`
  );
};
exports.generateMetrics = async (token, project_token) => {
  await exec(
